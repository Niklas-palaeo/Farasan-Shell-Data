...
---
#Introduction
This file explains the process behind the `Farasan Database.R` file, which is referred to in the other documents, which carry out the actual analysis and illustration. Is is not essential to understanding the dataset, as none of the data are changed, but it can help to understand how and why the data are structured as they are.
...
All .csv files are located in the folder *CSV Exports*. So the first step is to set the working directory to that folder. But before you do that, you need to change the file path, so that it fits with where you downloaded this folder. RStudio has an option in the "Files" panel which will tell you the whole command, but you will have to put it into the chunk below as well as wherever it comes up in the walkthroughs. A simple find&replace should do the trick!
...
Janaba_West <-
  list(#JW0001 to JW0021 are TestPits, which had already been combined into one list
    ...
    "JW0019" = TPs[[19]],
    "JW0021" = TPs[[20]],
    #Here is where the full columns at each site start
    "JW1705" = JW1705,
    "JW1727_A" = JW1727_A,
